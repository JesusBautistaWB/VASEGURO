...
<body>
        ...
        $sql="SELECT DISTINCT folioAccidenteServicio FROM vasegurobd.tab_serviciosadicionales  ORDER BY fechaSolicitud DESC";
        
		$result=mysqli_query($conexion,$sql);
        while($row = @mysqli_fetch_array($result)){
            $dir[$cont] = $row['folioAccidenteServicio'];
            $cont++;
        }

        foreach ($dir as $valor) {

            $con = "SELECT * FROM vasegurobd.tab_serviciosadicionales WHERE folioAccidenteServicio = '$valor' ";
            echo $con;
            $result2=mysqli_query($conexion,$con);
            $total = "";
            foreach ($result2 as $milista) {
