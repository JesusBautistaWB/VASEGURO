...
<body>
 ...
 <!-- TRAEMOS LA INFORMACION BASICA DEL USUARIO, YA QUE SE UTILIZARA POSTERIORMENTE PARA FOLIAR AL ACCIDENTE EN CASO DE APROBACION-->
  <form>
      ...
      <div class="col-3">
      <label>
      <input  placeholder="<?php  $fechaActual = getdate('Y-m-d')-8;
...
ACCIDENTES POR ESTATUS

<table>
  <tr><thead>
    <td>ESTATUS</td>
    <td>ACCIDENTES HOY</td>
    <td>ACCIDENTES ESTE MES</td>
    <td>TODOS</td>
</thead>
</tr>
<?php
        ...
        $sql="SELECT * FROM vasegurobd.cat_estatus;";
        $result=mysqli_query($conexion,$sql);    
      
          if($result->num_rows > 0){
              while($row = $result->fetch_assoc()){
                echo "<tr><td>".$row['nombreEstatus']."</td><td>".$row['registrosDia']."</td><td>".$row['registrosMes']."</td><td>".$row['registrosTotal']."</td></tr>";
              }
          }
          ?>
</table>
...
<table>
   ";
    ...
    INNER JOIN cat_estatus ON tb_accidentes.idEstatus=cat_estatus.idEstatus
    WHERE MONTH(fechaRepor) = MONTH(current_date()) AND  DAY(fechaRepor) = DAY(current_date())
    GROUP BY idHospital ORDER BY Accidentes DESC UNION SELECT
    idHospital AS Hospital, COUNT(idHospital) AS AccidentesM, fechaRepor
    FROM vasegurobd.tb_accidentes
    INNER JOIN cat_estatus ON tb_accidentes.idEstatus=cat_estatus.idEstatus
    WHERE MONTH(fechaRepor) = MONTH(current_date())
    GROUP BY idHospital ORDER BY Accidentes DESC UNION SELECT
    ...
    INNER JOIN cat_estatus ON tb_accidentes.idEstatus=cat_estatus.idEstatus
    WHERE tb_accidentes.idEstatus <> 0 
    GROUP BY Hospital ORDER BY AccidentesA DESC ;";
    $result=mysqli_query($conexion,$sql);   
    if($result->num_rows > 0){
      while($row = $result->fetch_assoc()){
        echo "<tr><td>".$row['Hospital']."</td>
        <td colspan='1'>".$row['Accidentes']."</td>
        <td colspan='1'>".$row['AccidentesM']."</td>
        <td colspan='1'>".$row['AccidentesA']."</td>
