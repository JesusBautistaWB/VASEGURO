...
echo '
<table id="tabFiles" name="tabFiles">
<tr>
<thead>
<td>Aprobar</td>
<td>ID</td>
...
foreach ($lista as $milista) {
    ...
    <td><a href='".$milista['ruta']."' target='_blank'>".$milista['ruta']."</td>
    <td>".$milista['tipo']."</td>
...
function enviarEgreso()
      {
         ...
         if(numCheck =="0" ){
            ...
            if(total == numCheck){
                alert("TODOS LOS ARCHIVOS HAN SIDO APROBADOS, EGRESO EXITOSO");
                window.location= "../functions/aprobacionTotal.php?folio="+folio+"&&ids="+ids;
            } else {

       alert("SE APROBARON: "+numCheck+" / "+total+" IDS:"+ ids + "NO:" + idsN);
      window.location= "../functions/verificarRutas.php?ids="+ids+"&&idn="+idN;  
            }
